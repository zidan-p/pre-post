...
export class Guard {
  ...
  public static greaterThan (minValue: number, actualValue: number): Result<GuardResponse> {
    return actualValue > minValue 
      ...
      : Result.fail<GuardResponse>(new ArgumentOutOfRangeException(`Number given {${actualValue}} is not greater than {${minValue}}`));
  }

  public static againstAtLeast (numChars: number, text: string): Result<GuardResponse> {
    return text.length >= numChars 
      ? Result.ok<GuardResponse>() 
      : Result.fail<GuardResponse>(new ArgumentOutOfRangeException(`Text is not at least ${numChars} chars.`));
  }

  public static againstAtMost (numChars: number, text: string): Result<GuardResponse> {
    return text.length <= numChars 
      ? Result.ok<GuardResponse>() 
      : Result.fail<GuardResponse>(new ArgumentOutOfRangeException(`Text is greater than ${numChars} chars.`));
  }
  ...
  public static isOneOf (value: any, validValues: any[], argumentName: string) : Result<GuardResponse> {
    ...
    if (isValid) {
      ...
      return Result.fail<GuardResponse>(new ArgumentInvalidException(`${argumentName} isn't oneOf the correct types in ${JSON.stringify(validValues)}. Got "${value}".`));
    }
  }
  ...
  public static inRange (num: number, min: number, max: number, argumentName: string) : Result<GuardResponse> {
    ...
    if (!isInRange) {
      return Result.fail<GuardResponse>(new ArgumentOutOfRangeException(`${argumentName} is not within range ${min} to ${max}.`));
    } else {
      return Result.ok<GuardResponse>()
    }
  }
  ...
  public static allInRange (numbers: number[], min: number, max: number, argumentName: string) : Result<GuardResponse> {
    ...
    if (failingResult) {
      return Result.fail<GuardResponse>(new ArgumentOutOfRangeException(`${argumentName} is not within the range.`));
    } else {
      return Result.ok<GuardResponse>()
    }
  }
